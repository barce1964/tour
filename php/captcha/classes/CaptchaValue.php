...
class CaptchaValue implements CaptchaInterface
{
    ...
    {
        $this->font_dir = $_SERVER['DOCUMENT_ROOT'] . '/php/captcha' . DIRECTORY_SEPARATOR . 'fonts' . DIRECTORY_SEPARATOR;
    ...
    public function captcha_image($code)
    {
        ...
        for ($i = 0; $i < $linenum; $i++)
        {
            $color = imagecolorallocate($image, rand(0, 180), rand(0, 100), rand(0, 180));
            imageline($image, rand(0, 180), rand(1, 70), rand(20, 180), rand(1, 70), $color);
        }
        ...
        for ($i = 0; $i < $pixels; $i++)
        {
            ...
            imagesetpixel($image, rand(0, 180), rand(0, 180), $color);
        }
